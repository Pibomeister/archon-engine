...
function displayUserMessage(
  ...
  expanded: boolean
): { text: string | null; long: boolean } {
  if (!message) return { text: null, long: false };
  const long = message.length > 80;
  return { text: expanded || !long ? message : `${message.slice(0, 80)}…`, long };
}

function StatusDot({ status }: { status: WorkflowRunStatus }): React.ReactElement {
  return (
    <div
      className={cn(
        'h-2.5 w-2.5 shrink-0 rounded-full',
        status === 'running' && 'bg-primary animate-pulse',
        status === 'paused' && 'bg-warning animate-pulse',
...
function NodeOutcomeSummary({ run }: { run: DashboardRunResponse }): React.ReactElement | null {
  if (
...
function UserMessagePreview({
  ...
  return (
    <div className="space-y-0.5">
      <p className={cn('text-xs text-text-tertiary italic', !expanded && 'truncate')}>{text}</p>
      {long ? (
        <button
          onClick={onToggle}
...
function ApprovalMessage({ run }: { run: DashboardRunResponse }): React.ReactElement | null {
  if (run.status !== 'paused' || run.metadata?.approval == null) return null;
  return (
    ...
    <div className="ml-auto flex items-center gap-1">
      ...
      {run.status === 'paused' && onReject ? (
        <ConfirmRunActionDialog
          ...
          description={
            <>
              Reject the paused workflow <strong>{run.workflow_name}</strong>. If the approval node
      ...
      {run.status === 'running' && onAbandon ? (
        <ConfirmRunActionDialog
          ...
          description={
            <>
              Mark <strong>{run.workflow_name}</strong> as cancelled. Already-completed nodes remain
              in the database; the run will not continue.
            </>
          }
...
export function WorkflowRunCard({
  ...
  const message = displayUserMessage(run.user_message, messageExpanded);

  return (
    <div className="rounded-lg border border-border bg-surface p-4 space-y-3">
      <RunHeader run={run} elapsed={elapsed} />
      <StepProgress run={run} liveState={liveState} />
      <NodeOutcomeSummary run={run} />
      <MetadataRow
        run={run}
        onParentChat={() => {
