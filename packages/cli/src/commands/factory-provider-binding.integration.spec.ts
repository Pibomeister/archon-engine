...
// Drives the factory provider path, which validateFactoryProviderScope qualifies to darwin and
// linux only: on Windows the workflow returns factory_provider_platform_unqualified instead of
// reaching the binding behaviour this asserts.
test.skipIf(process.platform === 'win32')(
  'native resume rejects changed qualified binding before bash and authorized successor retains it',
  async () => {
...
`
    ...
    expect(
      (await invoke(['run', 'binding-child', 'No authority', '--adopt', parentId], unconfigured))
        .code
    ).not.toBe(0);
    expect(existsSync(join(project, 'after-successor.txt'))).toBe(false);
    await writeFile(
    ...
    const childStatus = JSON.parse(
      (await invoke(['launch-status', 'child-authorized'])).stdout
    ) as {
      receipt: { runId: string };
    };
