...
// Drives a real native child through a `#!/bin/sh` executable made runnable with chmod 0o700.
// Windows has neither, so the spawn fails before the drain behaviour is reached.
test.skipIf(process.platform === 'win32')(
  'real SDK terminal event and stdout EOF cannot release before native process exit',
  async () => {
